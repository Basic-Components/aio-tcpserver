...
def tcp_server(
    ...
    """
    if costume_config:
        server_config = costume_config.get("SERVER_CONFIG") or SERVER_CONFIG
        server_logging_config = costume_config.get(
            "SERVER_LOGGING_CONFIG") or SERVER_LOGGING_CONFIG
    else:
    ...
    if worker is not None and worker > 2:
        logger.info('Multiple tcp server starting @{}:{}'.format(
            server_config["host"],
            server_config["port"])
        )
        ...
        logger.info('Single tcp server starting @{}:{}'.format(
            server_config["host"],
            server_config["port"])
        )
